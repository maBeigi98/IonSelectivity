function []=plotIvsVforAllCond(g,z,pHnum,condNum)
%
...
for i=1:g.numpH
    ...
    %make a line for each conductance
     for j=1:g.numCond
        ...
        if nargin>3 && j~=condNum;continue;end;
        hdata(c) = plot(g.(zap).m.Vs{i,j},g.(zap).m.Is{i,j});
        hdata(c).Color = map(j,:);
        hdata(c).Marker = markConc{i};
        hdata(c).LineStyle = 'none';
        hfit(c) = plot(g.(zap).m.linearfit{i,j});
        hfit(c).Color = map(j,:);
        %append to legend
        condStr   = [num2str(g.condConc(j)) ' M, G = ', num2str(g.(zap).m.cond(i,j)), ' (nS)', ' pH', num2str(g.pH(i))];
